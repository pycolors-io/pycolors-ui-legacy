...
They define the atomic decisions that every component consumes — ensuring consistency,
predictability, and safe global evolution of the design system.

Tokens are **not components** and **not stylesheets**.
They are **atomic design decisions expressed as variables**.

---

## What are tokens?

A token encodes **meaning**, not appearance.

In PyColors UI, tokens represent **roles**, not raw values. They answer questions like:
...
Design decisions become tokens, then utilities, then components:
...
UI Components (primitives)
  ↓
Guides & Patterns (real product UI)
```
...
Not everything is tokenized yet — intentionally.

PyColors UI starts with a minimal, high-impact token set.
Spacing, motion, and z-index may be introduced later once patterns stabilize.
